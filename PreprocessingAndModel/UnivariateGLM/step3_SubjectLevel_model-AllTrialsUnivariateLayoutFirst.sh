...
cat $FSL_TEMPLATES/univarAllSubjLevelLayoutFirst.fsf \
| sed "s|subjX|$subj|g" \
> $subj_design_dir/univar_SubjLevelLayoutFirst.fsf

#run feat -  model
if [ ! -d "$subj_output_dir/SubjLevelTasks.feat" ]; then

echo "running FEAT model on $subj"
feat $subj_design_dir/univar_SubjLevelLayoutFirst.fsf &
sleep 60
