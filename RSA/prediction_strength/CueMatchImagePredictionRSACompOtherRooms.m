function [ResultsCueImSim, ResultsCueImSimItemsCount, ResultsCueImSimOnlyNum, ResultsCueImSimAllTrials]=CueMatchImagePredictionRSACompOtherRooms(engram,remove_outliers)
...
results_fname=sprintf('CueMatchImagePredictionRSACompOtherRoomsWithRstruct_%s.mat', out_ttl);
...
for subj=1:numel(subjects)
    ...
    for reg=1:numel(reg_names)
        ...
        if strcmp(char(subjects(subj)),'EB') && (strcmp(reg_names{reg},'lacc') || strcmp(reg_names{reg},'acc') || strcmp(reg_names{reg},'racc'))
            ...
            %add the cues:
            ResultsCueImSimAllTrials.cue_act.(reg_names{reg}).all_items.(subjects{subj})=cue_act(curr_trials);
            ResultsCueImSimAllTrials.cue_act.(reg_names{reg}).Acc.(subjects{subj})=cue_act_acc(curr_trials);
            %compute univariate activation, 0-changes images, per task:
            task_all0_changes=task_binary(all_0changes);
            %layout task
            curr_im_trials=all_0changes(task_all0_changes==1);
            image_act=image_data(:,curr_im_trials);
            image_act=nanmean(image_act,1); %average voxels
            image_act_final=nanmean(image_act); %average images
            %item task
            curr_im_trials=all_0changes(task_all0_changes==0);
            image_act=image_data(:,curr_im_trials);
            image_act=nanmean(image_act,1); %average voxels
            image_act_final=[image_act_final nanmean(image_act)]; %average images
            ...
            ResultsCueImSimOnlyNum.AvPerTaskNo0Changes.(reg_names{reg}).intact_image_act.all_items(subj,:)=image_act_final;
